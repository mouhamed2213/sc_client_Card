...
function Hero({ fiche, actions }: FicheTemplateProps) {
  ...
  return <section className="public-hero">
    ...
    <div className="public-identity"><div className="public-avatar">{identityImage ? <img src={identityImage} alt={`${fiche.prenom} ${fiche.nom}`} /> : <span>{fiche.prenom.slice(0, 1)}{fiche.nom.slice(0, 1)}</span>}</div><p className="public-name">{fiche.prenom} {fiche.nom}</p><p className="public-role">{fiche.fonction}<span className="mx-2 text-white/30">·</span>{fiche.entreprise}</p></div>
...
function GalleryCarousel({ images }: { images: FicheTemplateModel["data"]["galerie"] }) {
  ...
  return <div className="pro-gallery"><div className="pro-gallery-stage"><img src={image.url} alt={image.alt} loading="lazy" />{gallery.length > 1 && <><button type="button" className="pro-gallery-control pro-gallery-control-prev" onClick={previous} aria-label="Photo précédente"><ArrowLeft className="h-4 w-4" /></button><button type="button" className="pro-gallery-control pro-gallery-control-next" onClick={next} aria-label="Photo suivante"><ArrowRight className="h-4 w-4" /></button></>}</div><div className="pro-gallery-meta"><span>Photo {current + 1} / {gallery.length}</span>{gallery.length > 1 && <div className="pro-gallery-dots" aria-label="Navigation de la galerie">{gallery.map((item, index) => <button type="button" key={item.url} className={`pro-gallery-dot ${index === current ? "is-active" : ""}`} onClick={() => setCurrent(index)} aria-label={`Afficher la photo ${index + 1}`} aria-current={index === current ? "true" : undefined} />)}</div>}</div></div>;
}
...
export function FicheTemplate({ fiche, actions, children }: FicheTemplateProps) {
  ...
  return <div className={`public-page fiche-template fiche-template--${config.theme}`} data-formule={fiche.formule}>
    <div className="public-card fiche-template__card"><Hero fiche={fiche} actions={actions} /><main className="public-content">
