...
import { ArrowUpRight, CalendarDays, Clock3, Download, ExternalLink, Globe2, MapPin, MessageCircle, Phone, Star, UserRound } from "lucide-react";
...
export type FicheTemplateActions = {
  ...
  whatsappHref: string;
  contactHref: string;
  buttonOrder: ("appel" | "whatsapp" | "contact")[];
  contactOpen: boolean;
  contactSent: boolean;
...
function Hero({ fiche, actions }: FicheTemplateProps) {
  const image = fiche.photo || fiche.logo;
  const buttons = {
    ...
    contact: <a href={actions.contactHref} download={`${fiche.slug}.vcf`} className="public-action public-action-contact"><Download className="h-5 w-5" /><span>Enregistrer</span></a>,
  } as const;
  return <section className="public-hero"><div className="public-topline"><span className="public-chip">Fiche vérifiée</span><span className="public-nfc">NFC · QR</span></div><div className="public-identity"><div className="public-avatar">{image ? <img src={image} alt={`${fiche.prenom} ${fiche.nom}`} /> : <span>{fiche.prenom.slice(0, 1)}{fiche.nom.slice(0, 1)}</span>}</div><p className="public-name">{fiche.prenom} {fiche.nom}</p><p className="public-role">{fiche.fonction}<span className="mx-2 text-white/30">·</span>{fiche.entreprise}</p></div><div className="public-actions">{actions.buttonOrder.map((key) => <span key={key}>{buttons[key]}</span>)}</div></section>;
}
  ...
  return <div className={`public-page fiche-template fiche-template--${config.theme}`} data-formule={fiche.formule}>
    <div className="public-card fiche-template__card">
      ...
      <main className="public-content">
        <div className="public-section intro-note"><span className="accent-line" /><p>Une fiche simple pour trouver la bonne information, sans détour.</p></div>
