...
export default function PublicFiche() {
  ...
  const visibleLinks = (data.liens ?? []).slice(0, plan?.maxLinks ?? (fiche.formule === "essentiel" ? 3 : 10));
  const visibleGallery = (data.galerie ?? []).slice(0, plan?.maxPhotos ?? 0);
  ...
  const identityImage = fiche.photo || fiche.logo;
  return <div className="public-page"><div className="public-card"><section className="public-hero"><div className="public-topline"><span className="public-chip">Fiche vérifiée</span><span className="public-nfc">NFC · QR</span></div><div className="public-identity"><div className="public-avatar">{identityImage ? <img src={identityImage} alt={`${fiche.prenom} ${fiche.nom}`} /> : <span>{fiche.prenom.slice(0, 1)}{fiche.nom.slice(0, 1)}</span>}</div><p className="public-name">{fiche.prenom} {fiche.nom}</p><p className="public-role">{fiche.fonction}<span className="mx-2 text-white/30">·</span>{fiche.entreprise}</p></div><div className="public-actions">{buttonOrder.map((key) => <span key={key}>{buttons[key as keyof typeof buttons]}</span>)}</div></section>
  <main className="public-content"><div className="public-section intro-note"><span className="accent-line" /><p>Une fiche simple pour trouver la bonne information, sans détour.</p></div>
    {(visibleLinks.length || fiche.site) ? <section className="public-section"><SectionTitle icon={<Globe2 className="h-4 w-4" />} title="Liens utiles" /> <div className="link-list">{fiche.site && <a href={fiche.site} target="_blank" rel="noreferrer" className="public-link"><span>Site internet</span><ExternalLink className="h-4 w-4" /></a>}{visibleLinks.map((link) => <a key={link.url} href={link.url} target="_blank" rel="noreferrer" className="public-link"><span>{link.label}</span><ExternalLink className="h-4 w-4" /></a>)}</div></section> : null}
    ...
    {(plan?.requiresHours && data.horaires?.length) ? <section className="public-section"><SectionTitle icon={<Clock3 className="h-4 w-4" />} title="Horaires" /><div className="hours-list">{data.horaires.map((row) => <div key={row.jour} className="hours-row"><span>{row.jour}</span><strong className={row.horaire.toLowerCase().includes("fermé") ? "text-[#a86155]" : ""}>{row.horaire}</strong></div>)}</div></section> : null}
    {hasForm ? <section className="public-section"><SectionTitle icon={<MessageCircle className="h-4 w-4" />} title="Être rappelé" />{contactSent ? <div className="rounded-xl bg-emerald-50 p-4 text-sm font-medium text-emerald-800">Votre demande a bien été transmise. L’établissement peut maintenant vous rappeler.</div> : contactOpen ? <form className="contact-form" onSubmit={(event) => { event.preventDefault(); contactMutation.mutate({ slug, ...contactForm }, { onSuccess: () => { setContactForm({ name: "", phone: "", message: "" }); setContactOpen(false); setContactSent(true); }, onError: () => undefined }); }}><input required minLength={2} placeholder="Votre nom" value={contactForm.name} onChange={(event) => setContactForm({ ...contactForm, name: event.target.value })} /><input required minLength={8} placeholder="Votre téléphone" value={contactForm.phone} onChange={(event) => setContactForm({ ...contactForm, phone: event.target.value })} /><textarea required minLength={2} placeholder="Votre message" value={contactForm.message} onChange={(event) => setContactForm({ ...contactForm, message: event.target.value })} /><div className="flex gap-2"><button type="button" className="public-secondary-link" onClick={() => setContactOpen(false)}>Annuler</button><button disabled={contactMutation.isPending} className="public-primary-link" type="submit">{contactMutation.isPending ? "Envoi…" : "Envoyer ma demande"}</button></div>{contactMutation.error && <p className="text-xs font-medium text-red-600">{contactMutation.error.message}</p>}</form> : <><p className="public-address">Laissez vos coordonnées, l’établissement vous recontactera directement.</p><button className="public-primary-link" onClick={() => setContactOpen(true)}>Demander un rappel <ArrowUpRight className="h-4 w-4" /></button></>}</section> : null}
    {(plan?.hasCatalog && data.sections?.length) ? <section className="public-section"><SectionTitle icon={<CalendarDays className="h-4 w-4" />} title="Carte & prestations" />{data.sections.map((section) => <div key={section.titre} className="catalog-section"><h3>{section.titre}</h3>{section.articles.map((article) => <div key={article.nom} className="catalog-row"><div><p className="font-semibold text-[#26344a]">{article.nom}</p><p className="mt-1 text-xs leading-5 text-[#8891a0]">{article.description}</p></div><span>{article.prix}</span></div>)}</div>)}</section> : null}
    {visibleGallery.length ? <section className="public-section"><SectionTitle icon={<UserRound className="h-4 w-4" />} title="Galerie" /><div className="gallery-grid">{visibleGallery.map((image) => <img key={image.url} src={image.url} alt={image.alt} loading="lazy" />)}</div></section> : null}
