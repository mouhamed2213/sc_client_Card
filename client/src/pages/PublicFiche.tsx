...
export default function PublicFiche() {
  ...
  useEffect(() => {
    if (fiche?.statut === "active") scanMutation.mutate({ slug });
  }, [fiche?.statut, slug]);
  ...
  const actions: FicheTemplateActions = {
    ...
    onContactFormChange: (field, value) => setContactForm((current) => ({ ...current, [field]: value })),
    onContactSubmit: (event) => {
      event.preventDefault();
      contactMutation.mutate({ slug, ...contactForm }, {
        onSuccess: () => {
  ...
  return <FicheTemplate fiche={templateFiche} actions={actions} />;
}
