...
const fichePayload = z.object({
  ...
  formule: z.enum(["essentiel", "pro", "signature", "commerce"]),
  ...
  data: z.object({
    ...
    liens: z.array(z.object({ label: z.string(), url: z.string() })).default([]),
    horaires: z.array(z.object({ jour: z.string(), horaire: z.string() })).default([]),
    galerie: z.array(z.object({ url: z.string(), alt: z.string() })).default([]),
    sections: z.array(z.object({ titre: z.string(), articles: z.array(z.object({ nom: z.string(), description: z.string(), prix: z.string() })) })).default([]),
...
export const appRouter = router({
  ...
  fiches: router({
    list: publicProcedure.query(async () => {
      ...
      return rows.map((row) => ({ ...parseFiche(row), plan: getPlanFeatures(row.formule) }));
    }),
    ...
    create: publicProcedure.input(fichePayload).mutation(async ({ input }) => {
