...
export const appRouter = router({
  ...
  fiches: router({
    list: publicProcedure.query(async () => { const rows = await listFiches(); return rows.map((row: Fiche) => ({ ...parseFiche(row), plan: getPlanFeatures(row.formule) })); }),
