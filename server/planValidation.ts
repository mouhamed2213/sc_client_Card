...
export function validatePlanPayload(input: {
  ...
  if (features.hasGoogleReview && !input.googlePlaceId)
    errors.push(
      "Commerce : google_place_id est obligatoire pour l'avis Google."
    );
  if (features.requiresHours) {
    const hours = input.data.horaires ?? [];
    if (hours.length !== 7)
      errors.push(
        "Commerce : les horaires doivent couvrir exactement les 7 jours."
      );
    if (hours.some(row => !row.jour?.trim() || !row.horaire?.trim()))
      errors.push(
        "Commerce : chaque jour doit avoir un horaire ou une mention Fermé."
      );
  }
  if (features.hasCatalog && !(input.data.sections ?? []).length)
    errors.push(
      "Commerce : au moins une section de catalogue est obligatoire."
    );
  return errors;
}
